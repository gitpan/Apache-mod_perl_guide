...
sub create_html_version{
  ...
  my $ra_ordered_srcs  = $config->get_param('pod_files');
    ...
    # Process each pod
  for (my $i=0; $i<@$ra_ordered_srcs; $i++) {

    my $file     = "$ra_ordered_srcs->[$i].pod";
    ...
    printf "+++ %-22s: processing ($rebuild_reason)\n",$file;
      ...
      # prev/next linking
    my $prev_page = ($i and defined $ra_ordered_srcs->[$i-1])
      ? "$ra_ordered_srcs->[$i-1].html" : '';
    my $next_page = (       defined $ra_ordered_srcs->[$i+1])
      ? "$ra_ordered_srcs->[$i+1].html" : '';
    my $curr_page = "$ra_ordered_srcs->[$i].html";
...
sub copy_the_rest{
  ...
  print "+++ Copying the non-pod files and dirs to $target_dir\n";
  ...
  foreach my $file (@$nonpod_files){
    system "cp -r $src_root/$file $target_dir";
  }
...
sub write_index_html_file{
  ...
  my $main_long_toc  = join "\n", 
    map {$self->{rh_main_toc}->{"$_.html"} } @ordered_srcs;
  my $main_short_toc = join "\n", 
    map {$self->{rh_main_toc}->{"$_.html"} =~ s|<UL>.*</UL>||ism;
	 $self->{rh_main_toc}->{"$_.html"} =~ s|<B><FONT SIZE=\+1>([^<]+)</FONT></B></A></LI><P>|$1</A></LI>|ism;
	 $self->{rh_main_toc}->{"$_.html"} =~ s^<P>^^gism;
	 $self->{rh_main_toc}->{"$_.html"} } @ordered_srcs;

  #  $self->{rh_main_toc}->{"$_.html"} =~ s^<B>|</B>|<P>^^gism;
  #  $self->{rh_main_toc}->{"$_.html"} =~ s^<FONT.*?>|</FONT>^^gism;

...
cancelled by a SIGPIPE in your Apache I<access_log>, you must define a
...
printing the NULL, we add $r->rflush(). Here is a new version of the
code:
...
happened to create external lock files and and use them for lock
...
requires an acquision of the exclusive lock), will be blocked waiting
...
The script reads in a file and writes it back, prepending a number of
...
to be prepended I<before> the file is locked.  Second, instead of
...
So you can see that I</tmp/io.pl> is actually close()'d.

Under Perl version 5.6 C<Symbol.pm>-like functionality is a built-in
...
modules use the C<Symbol> module themselves.  Here is an example of
their usage:

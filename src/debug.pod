...
requests while its status is 'W' (Which means that it hangs or "runs
away")

But when you have about 50 and more processes, it's quite hard to spot
such a process. So let's use a watchdog to do the work for us. I've
put together the
L<Apache::Watchdog::RunAway|modules/Apache_Watchdog_RunAway_Hang>
module which includes the C<amprapmon> utility whose name stands for
I<Apache mod_perl Run Away Processes Monitor>. This utility monitors
your server all the time and kills the processes that serve a single
request for too long. You define the threshold time for the kill-off.
...
handlers|dbm/Locking_dbm_handlers> to learn why you should NOT 
use a dbm's file descriptor to lock itself. To make long explanations
short -- it may get your dbm file corrupted.

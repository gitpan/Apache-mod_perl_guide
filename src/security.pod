...
against. For example, you can authorize upon IP number, allowing only
...
Once user passes the authentication stage, either bypassing it because
...
In the Perl API you would use note_basic_auth_failure() method to

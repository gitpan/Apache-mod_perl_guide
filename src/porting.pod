...
Declare a package name in the required files!  It should be unique to
the rest of the package names you use.  C<%INC> will then use the
unique package name for the key. It's a good idea to use at least
two-level package names for your private modules,
e.g. C<MyProject::Carp> and not C<Carp>, since the latter will collide
with an existing standard package.  Even if as of the time of your
coding it doesn't yet exist, a package might enter the next perl
distribution as a standard module and your code will be
broken. Foresee problems like this and save yourself future trouble.
...
In the C<Apache::Registry> script you would do:
...
Even if you don't need to call the child_terminate() at the end of the
request if you want the process to quit afterwards, here is an example
of assigning of the postprocessing handler, in case you might want to
...
I<Apache> module. This allows to run the CGI scripts unmodified under
C<Apache::Registry> and chaining the output of one content handler to
...
connection is broken the handler won't wait forever trying to send
data.
...
everytime. So mod_perl gives you the default set of headers, just like
...
the proper way to send the HTTP header is with send_http_header()
...
document body, but the HTTP header only. When Apache receives a HEAD
...
by matching the extension of the URI in the MIME tables (from the
...
in the C<Apache::Registry> C<<Location>> section of your
...
example, no matter of whether C<PerlSendHeader> is directive takes an
effect or not.
...
distribute, hopefully under one of the Open Source or GPL licences.
...
ones that run under mod_perl with enabled Taint mode and the rest on
another server with disabled Taint mode -- this can be either a
...
C<PERL5OPT>. Switches in this variable are taken as if they were on
...
=head1 Accessing Request Object in non-Perl*Handler Modules
...
In the C<Apache::Registry> scripts (and friends), the request object
is available through C<Apache-E<gt>request($r)> method. For example:
...
use these modules with script running under C<Apache::Registry> (and
...
If you run this example with circular reference enabled under mod_perl
you won't see C<Foo-E<gt>DESTROY> until the server shutdown, or until
your module properly took care of things. Note that the warn() call
logs its messages to the I<error_log> file, so you should expect the
printings there and not together with STDOUT.

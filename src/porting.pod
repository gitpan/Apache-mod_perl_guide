...
For example if we move wrap the code from the script into the
subroutine I<thecode>, place the both subroutines into the the
I<mylib.pl> file, save it in the same directory as the script itself
and require() it, there will be no problem at all. (Don't forget the
C<1;> at the end of the library or the require() might fail.)
...
This module is available from L<CPAN|download/CPAN_Downloads>. In one
of the next mod_perl releases, it should become a part of the CORE
distribution.
...
also insecure sinve its use exposes the data on systems which allow

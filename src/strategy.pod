...
At the beginning there were two servers: one plain Apache server, which
...
(e.g. http://www.example.com/images/test.gif) and C<httpd_perl>
listens to port 8080 (e.g. http://www.example.com:8080/perl/test.pl).
Note that I did not write http://www.example.com:80 for the first
...
to your site with 56 kbps (bps == bits/sec) modem. 1 byte == 8
bits. It means that the speed of the user's link is 56/8 = 7
kbytes/sec. I assume an average generated HTML page to be of 42KB (KB
== kilobytes) and an average script that generates this output in 0.5
second. How long will the server wait before the user gets the whole
output response? A simple calculation reveals pretty scary numbers --
it will have to wait for another 12 secs (42kb/0.5*7KB), when it could
serve another 11 (12/1-1) dynamic requests in this time.
...
speed of 7K/sec we were assuming before, may often be 5-10 times
slower.
...
servers. (This is called Load Balancing and it's a pretty big
...
application. A proxy server like squid is light in the sense of having
only one process serving all requests. But it can appear pretty heavy
when it loads objects into memory for faster service.
...
implied that squid only buffers in 16k chunks so it would not allow 
...
You have started your business as a small service providing web-site.
...
faster CPU and a faster hard disk.

The situation goes back to normal but not for a long, as the demand
for your services keeps on growing and just a little time after you've
upgraded your machine, once again it cannot cope the load. Should you
buy an even stronger and very expensive machine or start looking for
another solution?
...
parameters entered into an HTML form and submitted to the web server by
...
give instructions to modify or process the data.  In our case, an
Internet browser.
...
have little or nothing in common; different modules, no code
...
+ 4MB (unshared 2nd code set ). Under this scenario, eleven megabytes 
are shared and eight megabytes not.
...
Let's assume that we have 260 megabytes of RAM dedicated to the
webserver.
...
without changing the machine's hardware.
...
a 70:30 hits relation and that we have 260MB of available memory, we
...
=head1 Do Not Put mod_ssl into a mod_perl Server

...
information. If you fetch an html over SSL and have an C<<IMG>> tag
that fetches the image from the non-secure server, the image show
broken. This is true for any other non-secured object as well. Of
course if the generated response doesn't include any embedded objects,
like images -- this is not a problem.

...
connection is of 56 killo-bits per second (~56000 bits/sec). It's not
56 killo-bytes per second, but 7 killo-bytes per second, because 1
byte equals to 8 bits. So don't let the merchants fool you--your modem
gives you 7 killo-bytes per second connection at most and not 56
killo-bytes per second as one might think.

Another convention used in computer literature is that if you see 10Kb
it usually means 10 killo-bits and 10KB is 10 killo-bytes. So if you
see upper case C<B> it generally refers to bytes, and lower case C<b>
to bits (and C<K> of course means killo and equals to 1024 or to 1000

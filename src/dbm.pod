...
times of course, but as a rough guide consider 5,000 to 10,000 records
to be reasonable.
...
module. This module can store and restore the whole symbol table of
...
C<L<Tie::DB_Lock|dbm/Tie__DB_Lock>> module, which I will discuss in
...
a slightly different implementation, as I wrote it before David Harris
...
that this code still needs some testing, so B<be
...
If your CGI script was interrupted in the middle, the C<DESTROY> block will
take care of unlocking the dbm file and flush any changes. So your DB will be
safe against possible corruption because of unclean program termination.

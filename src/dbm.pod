...
The C<HASH> algorithm gives an C<0(1)> complexity of search and update,
fast insert and delete, but a slow sort (which you have to implement
yourself).

=item *

The C<BTREE> algorithm allows arbitrary key/value pairs to be stored
in a sorted, balanced binary tree.  This allows us to get 
a sorted sequence of data pairs in C<0(1)>, but at the expense of much 
slower insert, update, delete operations than is the case with C<HASH>.
...
you keep it open (tied) all the time, so that when your CGI script wants to
access the database it is already tied and ready to be used.  This will
...
There are several locking wrappers for C<DB_File> in CPAN right now.  Each
...
serial.  Use this for databases where reads may be lengthy and consistency
...
everything will be done automatically.  However it is better to use the
...
ASCII values to a dbm file. Then read the file and print the
...
If your CGI script is interrupted, the C<DESTROY> block will take 
care of unlocking the dbm file and flush any changes.  So your DB 

...
When the process is considered as I<hanging> it will be killed and the
event logged into a log file.
...
in your I<startup.pl> or your code:
...
Note: This module will run on platforms supported by B<GTop.pm> a Perl
interface to libgtop (which of course needs B<libgtop> : See
http://home-of-linux.org/gnome/libgtop/ ).

Referer to the C<Apache::GTopLimit> manpage for more information.
...
using filename instead of URI for the namespace.  It also uses an Object 
Oriented interface.
...
times.  Why can't we do the same with a plain ASCII (HTML, JS etc.)?
ASCII text can often be compressed by a factor of 10. 
...
C<Apache::GzipChain> keeps a list of user-agents, thus it also 
looks at the C<User-Agent> header to check for browsers known to 
...
sent, and compare that with the bytes sent using a regular configuration.

(See also C<perldoc Apache::GzipChain>).

Notice that the rightmost PerlHandler must be a content producer.  Here
we are using C<Apache::PassFile> but you can use any module which creates
output.
...
respond at I<access.log> or by telnet:
...
C<delete $INC{ $filename }>, unshift the preferred C<PerlINC> path into
...
If you can't find it on CPAN, get it at:

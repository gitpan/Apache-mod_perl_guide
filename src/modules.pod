...
time in seconds after which the process to be counted as I<hanging> or
I<run away>.

When the process is considered as I<hanging> it will be killed and the
event logged into a log file.

Generally you should use the C<amprapmon> program that bundled with
this module's distribution package, but you can write your own code
using the module as well. See the I<amprapmon> manpage for more info
about it.

Note that it requires an C<Apache::Scoreboard> module to work.
...
the output on the fly, the last adds the Embperl processing.
...
here the C<Apache::PassHtml> module simply picks the file's content
and sends it to STDOUT ... here it's processed by C<Apache::SSIChain>
...  and then goes to C<Apache::OutputChain> that sends the result to
the browser.

Alternative to this approach is C<Apache::Filter>, which has more
natural "forward" configuration order and it's easier to make other
modules to be compliant with C<Apache::Filter>.

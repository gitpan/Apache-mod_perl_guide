...
C<httpd.pid> file.  It's easy to discover where to look at, by
checking out the I<httpd.conf> file.  Open the file and locate the entry
...
Another way is to use the C<ps> and C<grep> utilities. Assuuming that
...
This will produce a list of all the C<httpd_perl> (parent and children)
processes.  You are looking for the parent process. If you run your
server as root, you will easily locate it since it belongs to root. If
you run the server as some other user (when you L<don't have root
access|install/Installation_Without_Superuser_P>, the processes will
belong to that user unless defined differently in I<httpd.conf>.  It's
still easy to find which is the parent - it's the smallest one.
...
Note that to send these signals from the command line the C<SIG>
...
to a mod_perl enabled server.  See
http://www.apache.org/docs/stopping.html for documentation on the
implications of sending these signals to a plain Apache server.
...
Sending the C<TERM> signal to the parent causes it immediately to
...
quite a few seconds to complete.  To stop a child, the parent sends
C<SIGHUP> signal.  If that fails it sends another.  If that fails it
sends C<SIGTERM> signal, and as a last resort it sends C<SIGKILL>
signal.  For each failed attempt to kill a child it makes an entry in
the I<error_log>.

Finally the parent itself exits and any open log files are closed.
This is when all the accumulated C<END> blocks, but the ones from
C<Apache::Registry> or C<Apache::PerlRun> scripts, will be
executed. The latter gets executed after each request is served.
...
then the parent not restart but will exit with an error and your sever
will be stopped.  See below for a way of avoiding this.


=item USR1 Signal: graceful restart

The C<USR1> signal causes the parent process to advise the children to
exit after serving the current request, or to exit immediately if
...
quite a long time for a mod_perl server.  You have an option to set
the L<PERL_DESTRUCT_LEVEL Environment
...
Apache's distribution provides a script to control the server.  It's
called I<apachectl> and it is installed into the same location as the
httpd executable.  Let's say in our scenario it's in
...
and you don't want to open any security holes.)
...
remotely, this ensures that if I suddenly get a lost connection (sadly
that happens sometimes) I won't leave the server down if only the
...
Also you have noticed that I use C<restart> and not just C<start>.  I
do this because of Apache's potentially long stopping times (it
depends on what you do with it of course!).  If you use C<start> and
Apache hasn't yet releases the port it's listening to, the start would
fail and C<error_log> would tell you that port is in use, e.g.:
...
there's a problem, I just put everything as it was before:
...
maintenance work on your database server.  If you have to take it down
the scripts using this database server will fail.
...
A much more friendlier approach is to confess to your users that
you are doing some maintenance work and plead for patience, promising
(keep the promise!) that the service will become fully functional in X
minutes.  There are a few ways to do this:

The first doesn't require messing with the server.  It works when you
have to disable a script, but not a module!  Just prepare a little
script like this:
...
in the C<<Location>> or C<<Directory>> section of your C<httpd.conf>.
...
configure a whole directory to be handled by a C<Construction> handler
(which you must write).  For example if you write something like this:
...
and put it in a directory that in the server's C<@INC>, to disable all
...
Now restart the server.  Your user will be happy to go and read
slashdot.org for ten minutes, knowing that you are working on a much
better version of the service.
...
This script is SUID I<root>, and should be executable only by members
of some special group at your site.  Note the tenth line, which fixes
an obscure error when starting apache/mod_perl by setting the real to
the effective UID.  As others have pointed out, a mismatch between the
...
by hand when you need it.  On a production system, there is chance
that the machine the server is running on will have to be rebooted.
When the reboot is completed, who is going to rememeber to start the
server?  It's easy to forget this task, and what happens if you aren't
around when the machine is rebooted?
...
have more than one Apache server you need a script for each one, and
of course you will have to rename them.
...
used for executing the scripts in a particular order: the lower
numbers are executed earlier.

When a machine is booted and its runlevel set to 3
...
Most of the systems have GUI utilites to automate the creation of
...
with multiple error messages, in five minutes you might run out of
free disk space which might bring your system to its knees.  Chances
are that no other child will be able to serve requests, since the
system will be too busy writing to the C<error_log> file.  Think big
-- if you are running a heavy service (which is very fast since you
are running under mod_perl) adding one more request every minute will
not be felt by the server at all.
...
module in httpd.conf.  In the I<access.conf> of our C<httpd_docs>
...
So if written in some I<file.html> I have a relative URL like
...
You have another problem: the CGI script may generate some html which
the client may then use to request further action from the server.  If
the script generates a URL with a hard coded PORT, the above scheme
will not work. There two solutions:

First, generate relative URLs so it will reuse the technique above,
with redirect (which is transparent for the user).  But this will not
...
the script runs from the shell but dies when called as a CGI.  The
...
lines into your I<error_log> file and in a few minutes to bring your
server to its knees.  For example sometimes I get bursts of an error
C<Callback called exit> showing up in my I<error_log>.  The file grows
300 Mbytes in a few minutes.  You should run a cron job to make sure
this does not happen, and if it does to take care of it.  Andreas

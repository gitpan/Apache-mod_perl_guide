...
status requests, but allow them only from browsers at I<example.com>,
you might use:
...
with I</home/httpd/foo/>. So when Apache gets a request
http://www.example.com/foo/test.pl the server will remap this into the
file I<test.pl> in the directory I</home/httpd/foo/>.
...
I</home/httpd/perl/test.pl>.  This means that we can have all our CGI
...
C<E<lt>DirectoryE<gt>> or C<E<lt>FilesE<gt>> section, it is an alias
for C<PerlPostReadRequestHandler>.  When outside any such section it
is an alias for C<PerlHeaderParserHandler>.
...
To retrieve the values use the C<get()> method:
...
C<$ENV{ORACLE_HOME}> (or another evironment variable that you need in
...
C<CGI.pm>'s C<compile()> method performs this task. Notice that this is a
...
just parse the code in I<startup.pl>, it actually executes it.
...
[META: this is incomplete. Should add C<ProxyPassReverse> to the front
end (to rewrite the hostname for external redirects),
and Port 80 to the backend for scripts that use C<SERVER_PORT> env var]
...
but use explicitely disjoint URL namespaces for C<Alias> and
C<Rewrite>. In other words any URL regexp that can potentially match a
...
I<http://www.example.com/> as their URL on the way back to the client.

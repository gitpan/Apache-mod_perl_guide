...
the script.  It is only meant for CGI emulation, and to send the HTTP
header it's always better either to use the C<$q-E<gt>header> method
from the C<CGI.pm> module or to use the C<$r-E<gt>send_http_header>
method using the Apache Perl API.
...
L<Apache::PerlVINC|modules/Apache_PerlVINC_Allows_Module>

...
mod_perl enabled Apache on a pretty average processor with a decent
amount of system memory?  It goes like this:
...
* The GNU C<tar> utility knows how to uncompress (the C<z> flag).
...
want to build mod_perl as loadable object instead of compiling it into
Apache, so that it can be upgraded without rebuilding Apache itself.
...
configuration parameters are available to you and when to use them.
...
available under Unix.  It doesn't work on the Win32 platform.
...
loaded via the C<LoadModule> directive in Apache's C<httpd.conf> file.
The benefit is that the modules are part of the C<httpd> program only
on demand, i.e. only when the user wants a module is it loaded into
the address space of the C<httpd> executable.  This is interesting for
instance in memory consumption and upgrade issues.
...
when DSO is enabled for module C<mod_xxx> via C<configure
...
can say C<APXS> is for Apache what C<MakeMaker> and C<XS> is for Perl.
It knows the platform dependent build parameters for making DSO files
...
the C source files of mod_perl which have to be compiled into the
C<httpd> program.  They are usually copied to the subdirectory
C<src/modules/perl/> in the Apache source tree.  To integrate this
subtree into the Apache build process a lot of adjustments were done
by mod_perl's C<Makefile.PL> in the past.  And additionally the
C<Makefile.PL> controlled the Apache build process.

This approach is problematic in several ways.  It is not very clean.
Because it is assumed that mod_perl is the only third-party module
which has to be integrated into Apache, it is very restrictive.

The new approach described below avoids these problems.  It prepares
only the C<src/modules/perl/> subtree inside the Apache source tree
...
will learn what they are, and when should they be used.
...
off.  You may enable them by editing I<src/modules/perl/Makefile>, or
when running C<perl Makefile.PL>.
...
nicely keep it along with the rest of the dot files in your home
directory.
...
you will start the server, by reporting an unknown directive.
...
Where C<Configuration.custom> is the full pathname of the file
I<relative to the Apache source tree you build against>.
...
Makefile.PL> stage, when the installed Perl was built with the C<gdbm>
...
the httpd binary when C<make> is called from Apache source directory
or loaded later, at run time.

Note: don't put the mod_perl directory inside the Apache directory,
since C<Apache::src> won't work then!
...
the test fails, the C<make test> stage will fail.
...
but of course the requirements may change in the future.
...
modules, scripts and documentation.  See http://cpan.org for more
information.
...
to make the use easier. The installation is as simple as typing:

  cpan> install Bundle::CPAN

The C<CPAN> shell will download mod_perl for you, unpack it, check for
the prerequisites, detect any missing third party modules, and
...
the binary in the appropriate directories.

The only caveat of described process is that you don't have control
over the configuration process.  Actually, that problem is easy to
solve -- you can tell <CPAN.pm> to pass whatever parameters you want
to C<perl Makefile.PL>.  You do this with C<o conf makepl_arg>
command:
...
C<perl Makefile.PL>.  If you add the
C<APACHE_SRC=/usr/src/apache_x.x.x/src> and C<DO_HTTPD=1> parameters,
you will be not asked a single question.  Of course you must give the
...
it somewhere so that you can restore it when you are done with the
...
bundle is a handy solution.  To accomplish that the command
...
installation, upgrade and cleanup.  One of the most well-used
...
setup hardware is less critical).  Let's say that one is a live server
and the other is in development.  You build an RPM with a mod_perl
binary distribution, install it on the development machine and find it
working and stable.  You can install the RPM package on the live
server without any fear.  Make sure that I<httpd.conf> is correct,
since it generally includes parameters such as hostname which are
unique to the live machine.
...
META: Currently only RPM.  Please submit info about DEB and other
available packages.
...
API will not function properly.  You may want to run the below script
...
of Perl, operating system, processor or chipset architecture, then the
...
Perl with C<XS> or C<SWIG>, are supposed to go into C<i386-linux> like
directories.
...
commands by terminating line with a backslash (C<\>), like this:
...
If there is no the C<CPAN> shell configured on your system, when you
...
resides) with C</home/stas>.  I use Perl of course!
  ...
  % perl -pi -e 's|/usr/src|/home/stas|' /home/stas/.cpan/CPAN/MyConfig.pm
...
=head2 Local mod_perl Enabled Apache Installation
...
C<tcsh> users will have to list all the parameters on a single line.
...
startup file which is C<require>'d before loading any other module that
resides in your local repository.  A much nicer approach is to use the
C<lib> pragma as we saw before, but in a slightly different way - we
use it in the startup file and it affects all the code that will be
executed under mod_perl handlers.  For example:
...
does script compilation.  As a result, C<@INC> will be reset to its
...
on a piece of paper instead of saving it to makepl_arg.save, but you
...
able to load dynamically Perl Modules extensions, make sure you answer
...
find libraries for ndbm, gdbm, db, etc., for the *DBM*_File modules.
...
C<ExtUtils::Embed> module returns these libraries to be linked with
...
a look at this order with the following command:
...
provide is implemented in C<Apache::{*}> Perl modules, it was reported
...
be displayed with C<Perl -V> command.  All these options are
...
either won't compile, it will dump core when run or maybe it will
behave in the most unexpected ways.

Since mod_perl uses Perl, Apache and third party modules, and they all
work together, it's a must to use the same compiler while building

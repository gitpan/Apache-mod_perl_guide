...
much memory becomes unshared.  You should make some measurements to
...
Notice that we have gotten the environment variable I<TEST> set.
...
external program to run if it's a Perl program. Remember that
...
[C-library].  Only when the command has shell meta-chars does Perl
invoke a real shell.  That's a I<very> nice optimization.  In other
words, only if you do:
...
Notice that the shell will be called also in the case where system()
accepts one single argument (i.e. C<system("echo foo")>) and not the
list of arguments (i.e. C<system("echo","foo")>). The shell will be
invoked also in the case where shell meta characters will be used
(i.e. C<system("echo foo*")>).
...
quits, it reports about the termination to its parent.  If no parent
...
=head1 -DTWO_POT_OPTIMIZE and -DPACK_MALLOC Perl Build Options
...
=head1 -Dusemymalloc Perl Build Option
...
It seems that Linux is still defaults to system malloc, you might want
to configure Perl with -Dusemymalloc. However Perl's malloc is not as
...
time, C<-A> -- last access time, C<-C> -- last inode-change time, and
others), the information is cached.  If you need to make an additional
check for the same file, use the C<_> variable and save the overhead
of this check.  For example when testing for existence and read
...
=head1 Keeping the Shared Memory Limit
...
have too little of shared memory. See
...
C<STACK>, C<FSIZE>, C<CORE>, C<MEMLOCK>; all
others are treated as their natural unit.  Prepend C<PERL_RLIMIT_> for
each one you want to use.  Refer to the C<setrlimit> man page on your
OS for other possible resources.
...
tens milli-seconds as you get with the C<Benchmark> module, you will
...
=head2 Benchmarking PerlHandlers
...
just like you'd use the C<Benchmark> module to benchmark a regular
Perl script.  Of course you can extend this module to make more
advanced processing like putting the results into a database for a
later processing.  But all it takes is adding this configuration
...
PerlHandler these are getting benchmarked as well.

An example of the lines showing up in the I<error_log> file:
...
set to 50: 50x20Mb = 1Gb.  Do you have 1Gb of RAM?  Probably not.  So
...
modules.  By using any of the two modules you should be able to
...
latter module allows to kill the servers whose shared memory size is
going below specified limit.
...
The above configuration allows us ot make a request to I</foo/test>
...
Now we modify the C<<Directory>> entry and add S<AllowOverride None>,
...
Let's see the same configuration using the C<<Perl>> section and a
dedicated package:
...
in the normal way (no C<<Perl>> section required.
...
perl5.6+ does a better method caching, C<Foo-E<gt>method()> is a
...
It has all the functionality of C<CGI.pm> except HTML generation
functions.
...
you don't really like, or it might be less convenient.
...
simple.  Do you NEED to cut out that overhead?  We don't know, your
...
used in the CGI coding. Mainly it has two parts -- the input
...
'use CGI qw(-compile => ':all') adds about 1Mb size to the server.
...
costs to us to use them. (perl5.6.0 over OpenBSD)
...
It seems that C<CGI.pm> suffers from the same decease:
...
v5.6 of Perl on four different operating systems, The three calls each
are without any modules, with just -MCGI, and with -MIO (never with
both):

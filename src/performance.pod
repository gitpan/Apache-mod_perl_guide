...
=head1 Performance: The Big Picture
...
not just Apache, and it is very important for you to understand it.
...
A webserver is only one of the elements the packets see along their
...
ultra fast, so you will need only a small number of working servers,
but you might find that the server processes are all busy waiting for
slow clients to accept their responses.  You will see more examples in
this chapter.

My point is that a Web service is like a car, if one of the parts or
mechanisms is broken the car may not go smoothly and it can even stop
dead if pushed too far without first fixing it.



=head1 Analysis of Software and Hardware Requirements
...
a professional perl (mod_perl) programmer to exercise your code and
improve it.
...
throwing another Ultra 2 into the rack.
...
=head1 Sharing Memory

The sharing of memory is another very important factor.  If your OS
...
its memory pages tend to become unshared.  There is no way we can make
Perl allocate memory so that (dynamic) variables land on different
memory pages from constants, so the B<copy-on-write> effect (we will
explain this in a moment) will hit you almost at random.
...
them.  The ideal is a point where your processes usually restart
before too much memory becomes unshared.  You should make some
measurements to see if it makes a real difference, and to find the
range of reasonable values.  If you have success with this tuning the
value of C<MaxRequestsPerChild> will probably be peculiar to your
situation and may change with changing circumstances.
...
RAM by using a lower value. Do not forget that if you preload most of
your code at server startup, the fork to spawn a new child will be
very very fast, because it inherits most of the preloaded code and the
perl interpreter from the parent process.  During the life of the
child its memory pages (which aren't really its own to start with, it
uses the parent's pages) gradually get `dirty' - variables which were
originally inherited and shared are updated or modified -- and the
I<copy-on-write> happens.  This reduces the number of shared memory
pages, thus increasing the memory requirement.  Killing the child and
spawning a new one allows the new child to get back to the pristine
shared memory of the parent process.

The conclusion is that C<MaxRequestsPerChild> should not be too big,
otherwise you lose some of the benefit of the memory sharing.
...
=head1 How Shared Is My Memory?

You've probably noticed that the word shared is repeated many times in
many things related to mod_perl.  Indeed, shared memory might save you
...
=head1 Calculating Real Memory Usage

You have learned how to measure the size of the process' shared
memory, but we still want to know what is the real memory usage, which
is obviously cannot be calculated by summing up the memory size of
each process, because it doesn't take into account the shared memory.

On the other hand we cannot just substruct the shared memory size from
...
probably would be a tough goal to give the exact number, but I've
found a way to get a pretty close number which was verified in the
following way. I have calculated the real memory used, by the
technique you will see in the moment, and then have stopped the Apache
server and saw that the memory usage report indicated that the total
used memory went down by almost the same number I've calculated. Note
that some OSes do smart caching so you may not see the memory usage
decrease, when it actually happens.
...
by all httpd processes, but the parent process.
...
trying to manually calculate this number use this module to do the
job.



=head1 Is my Code Shared?

How do you learn whether the code you write is shared between the
process or not. Well, not all of the code, but some variables. For
example, if you have some variables that use a lot of memory and you
want them to be read-only. As you know the variable becomes unshared
when the process modifies its value.
...
called I<MyShared.pm> which we preload at the server startup, so all
...
the tree functions. The goal is to check whether pos() makes the
...
Now open two browser windows and issue the request for this script for
a several times in both windows, so you get different processes PID
...
=head1 Preload Perl Modules at Server Startup
...
by defering the compilation phase.  When you use mod_perl, FastCGI or
another system that uses a persistent Perl interpreter, you will want
to precompile the methods at initialization time.  To accomplish this,
...
CGI-E<gt>compile(':all') help us?  We probably do not use all the methods
...
=head1 Preload Registry Scripts
...
only a first impression and needs better investigation.  If you aren't
concerned about occasional script invocations taking a little time to
respond while they load the code, you might not need it at all!

See also L<BEGIN blocks|porting/BEGIN_blocks>


=head1 Global vs Fully Qualified Variables 

It's always a good idea to avoid global variables when possible.  Some
variables must be either global, such as a module's C<@ISA> or
...
reduces a bit of noise.  However, C<vars> pragma also creates aliases,
...
=head1 Avoid Importing Functions
...
memory.  Try to use method interfaces and fully qualified names like
...
Apache API no longer need this extra C<%ENV> population, can gain by
turning it B<Off>.

By default it is On.

Note that you can still set ENV variables.  For example when you use
...
A script having a C<print Data::Dumper(\%ENV)> line, prints:
...
=head1 Proxying the mod_perl Server
...
=head1 Caching Components with HTML::Mason
...
you have a real performance penalty, since after completing each
request processing, the process will wait for C<KeepAliveTimeout>
seconds before closing the connection and thus not serving other
requests at this time.  With this configuration you will need many
...
=head1 Upload/Download of Big Files
...
=head1 Forking and Executing Subprocesses from mod_perl

Generally you should not fork from your mod_perl scripts, since when
...
C<setsid()>).  This is wise only if the parent which spawns this
process immediately continues, and does not wait for the sub-process
to complete.  This approach is suitable for a situation when you want
to use the Web interface to trigger a process which takes a long time,
such as processing lots of data or sending email to thousands of
users.  Otherwise, you should convert the code into a module, and call
its functions or methods from CGI script.

Just making a C<system()> call defeats the whole idea behind mod_perl.
The Perl interpreter and modules should be loaded again for this
...
commas, shell won't try to parse (tokenize) the parameters, therefore
you don't have to worry about escaping unsafe shell characters. If you
want shell to parse the variables make sure to run the escape
function, for example the one from the C<String::ShellQuote> package.
...
directly.  Perl system() is *not* C<system(3)> [C-library].  Only when
the command has shell meta-chars does Perl invoke a real shell.
...
Since it is C<echo>, it will execute it as a built-in in the latter
example or as C</bin/echo> in the former and be done, but this is only
an example.  You aren't calling C<system("echo Hi")> in your mod_perl
scripts, right?
...
you execute new code.  The only place you will see real overhead from
this scheme is when the parent process is huge (unfortunately like
mod_perl...) and the page table becomes large as a side effect.  The
whole point of mod_perl is to avoid having to fork() or exec()
something on every hit.  Perl can do just about anything by itself.
However, you probably won't get into trouble until you hit about 30
forks/second on a so-so pentium.
...
zombie.  A zombie is a process that doesn't have a parent.  When the
child quits, it reports the termination to its parent.  If no parent
...
C<<defunc>> tag, and you will see the zombies counter increment when
...
continue because you will get yet another zombie process.
...
processes will be collected by the C<init> process and prevent from
becoming zombies.
...
append to the error_log file as inhereted by parent, so chances are
that you will want it to leave untouched.

Of course if your child needs any of the C<STDIN>, C<STDOUT> or
C<STDERR> you should reopen them. But you must unties the parent
process, thus you should close them first.
...
Note that the last two solutions do allow you to know the exit status
of the process, but in our case we don't want to.

One more solution is to use a different I<SIGCHLD> handler:
...
When finished, overwrite the source file.  Make sure to provide a
locking mechanism!

=head2 Copying Variables Between Functions
...
Once I wrote a script that passed the content of a little flat file
...
the DB--refer to C<perldoc DBI> for a complete manual of the C<DBI>
module):
...
possible way.  The example below prints a HTML TABLE with matched
...
=head1 -DTWO_POT_OPTIMIZE and -DPACK_MALLOC Perl Options
...
by about 150k -- quite big number if you remember to multiply it by the
number of chidren you use.

C<-DTWO_POT_OPTIMIZE> macro improves allocations of data with size
...
=head1 Checking script modification times

Under C<Apache::Registry> the requested CGI script is always being
C<stat()>'ed to check whether it was modified. It adds a very little
overhead, but if you are into squeezing all the juice from the server,
you might want to save this call.  If you do, take a look at
L<Apache::RegistryBB|modules/> module.
...
=head1 Cached stat() calls

When you do a stat() (or its variations C<-M> - modification time,
C<-A> last access time, C<-C> inode-change time, and others), the
information is cached.  If you need to make an additional check for
the same file, use the C<_> variable and save the overhead of this
check.  For example when testing for existence and read permissions
you might use:
...
the C<-M> and similar file tests.  C<-M> returns the difference in
time between the modification time of the file and the time the script
was started.  Because the C<^T> variable is not reset on each script
invocation, and is equal to the time when the process was forked, you
...
=head1 Be carefull with symbolic links
...
after the both URIs have been requested from the same child process
...
=head1 Limiting the Size of the Processes

C<Apache::SizeLimit> allows you to kill off Apache httpd processes if
they grow too large.  See perldoc C<Apache::SizeLimit> for more
details.
...
=head1 Limiting the Resources Used by httpd Children
...
=head1 Limiting the Request Rate Speed (Robot Blocking)
...
=head1 Benchmarks. Impressing Your Boss and Your Colleagues.
...
numbers below.  Checkout the C<benchmark> directory of the mod_perl
...
=head2 Benchmarking a Graphic Hits Counter with Persistent DB Connections
...
=head2 Benchmarking Scripts with Execution Times Below 1 Second

As noted before, for very fast scripts you will have to use the
C<Time::HiRes> module, its usage is similar to C<Benchmark>'s.
...
=head2 Benchmarking PerlHandler

At http://perl.apache.org/dist/contrib/ you will find the
C<Apache::Timeit> package which does C<PerlHandler> Benchmarking.
...
=head1 Tuning Apache's Configuration Variables for the Best Performance
...
you run too many servers (not B<far> too many, of course) since the
processes are about 1Mb and don't eat a lot of your RAM.  Generally
the numbers are even smaller with memory sharing.  The situation is
different with mod_perl.  I have seen mod_perl processes of 20Mb and
more.  Now if you have C<MaxClients> set to 50: 50x20Mb = 1Gb.  Do you
have 1Gb of RAM?  Probably not.  So how do you tune the parameters?
Generally by trying different combinations and benchmarking the
server.  Again mod_perl processes can be of much smaller size with
memory sharing.
...
mod_perl scripts you possess.  You need it to have an ability to
...
which comes with apache distribution, the L<crashme
...
10000 -c 500> resulted in 94.82 reqests per second.  The server's
...
important for user response times.  Sometimes user will have to wait a
bit.
...
C<MaxRequestsPerChild> should be as big as possible, to take the full
...
that the performance will be better.  You have just see the numbers!
...
(27,000/150), independent of what load the server can actually
...
more than C<MaxClients> concurrent users at any time.  This situation
is accompanied by the following warning message in the C<error_log>:
...
will need to get more RAM so you can start more children.  The best
approach is to try not to have this condition reached at all, and if
you reach it often you should start to worry about it.
...
you come up with solution please let us know!  If the shared memory
...
Another approach is to use C<Apache::SizeLimit>.  See L<Limiting the
size of processes|performance/Limiting_the_Size_of_the_Process>.
...
C<Apache::SizeLimit>, you can set this parameter to 0 (equal to
infinity). You will want this parameter to be smaller if your code
becomes unshared over the process' life.
...
=head1 Persistent DB Connections
...
=head2 Preopening Connections at the Child Process' Fork Time
...
=head2 Caching prepare() Statements
...
=head2 Handling Timeouts

Some databases disconnect the client after a certain time inactive.
This problem is known as the B<morning bug>.  The C<ping()> method
ensures that this will not happen.  Some C<DBD> drivers don't have
this method, check the C<Apache::DBI> manpage to see how to write a
C<ping()> method.
...
it to 36 hours will probably fix the timeout problem in most cases.

=head1 Jeff's guide to mod_perl database performance

=head2 Analysis of the Problem
...
procedure.  There are several steps that need to be done to complete
the request:
...
=head2 Optimizing Database Connections
...
=head2 Utilizing the Database Server's Cache
...
=head2 Eliminating SQL Statement Parsing
...
load balancing.  If there are any suggestions for further optimization
of the application-database interaction, please mail them to me at
jwb@cp.net.

Jeffrey Baker,
4 October 1999

=head1 Using $|=1 Under mod_perl and Better print() Techniques.
...
empty line to be C<<HTML>> with *no* spaces before the opening left
angle-bracket.  If there are any othere characters, they might not
accept the output as HTML and print it as a plain text.  Even if it
works with your browser, it might not work for others.
...
to complete, I want users to get some titles while they are waiting.
...
stars" for a while and then to receive the whole output at once, even
if it's a few milliseconds faster - assuming the browser didn't time
out during the wait.

=back

An even better solution is to keep the buffering enabled, and use a
...
user in the buffer, and flush it a moment before you are going to do
some lenghty operation, like DB query.  So you kill two birds with one
...
=head1 More Reducing Memory Usage Tips
...
=head1 Measuring the Subroutines Memory Usage

With help of C<Apache::Status> you can check what is the size of each
and every subroutine.

=over

=item * Build and install mod_perl as you always do, make sure it's a
version 1.22 or higher.

=item * Configure /perl-status if you didn't before:
...
=item * From your favorite Netscape browser fetch
http://localhost/perl-status

=item * Click on 'Loaded Modules' or 'Compiled Registry Scripts'

=item * Click on module or script of your choice (you might need to run some
...
For example let's compare OO vs methods C<CGI.pm>'s interfaces:

As you will see below the first OO script uses about 2k bytes while
the second script (methods interface) uses about 18k.
...
After executing each script in a single server mode (-X) that's what
I've seen:
...
=item 2
  ...
  server_port           123 bytes |  0 OPs
   [many more report lines truncated]
...
=head1 Memory Swapping is Considered Bad
...
software that you run in a way that no memory swapping will
happen. Even during the peak hours.

Swap memory is slow since it resides on the hard disc, which is much
slower than the RAM. When your machine starts to swap, because it's
unable to cope with a number of the processes it has to run, your
machine will become slower and slower until it'll completely halt its
operations. When the CPU has to page in and out the memory pages,
things slow down, causing the processing demands to go up, which in
turn slows down the system even more as more memory is required and
this is provided by kerner using the reserved swap space.

This is the path to the machine halt, unless the resource demand is
suddenly goes down and allows the processes to catch up with their
tasks and go back to the normal usage of memory.

For the swapping monitoring techniques see the section
'L<Apache::VMonitor -- Visual System and Apache Server
Monitor|debug/Apache_VMonitor_Visual_Syste>'.

For the mod_perl specific swapping preventing guideliness see the
...
snippets of code take the longest execution time and which subroutines
...
the child is shutdown (where C<$$> is a number of the child
process).  All it takes is to add to C<httpd.conf>:

  PerlModule Apache::DProf

Remember that any PerlHandler that was pulled in before
C<Apache::DProf> in the C<httpd.conf> or <startup.pl>, will not have
its code debugging info inserted.  To run C<dprofpp>, chdir to
...
=head1 Object Methods Calls Versus Function Calls

Which subroutine calling form is more efficient: OOP methods or
functions?

=head2 The Overhead with Light Subroutines

Let's do a benchmarking.  We will start doing it using empty methods,
...
first name, I<function> does this explicitly, while I<method> does
this transparently.
...
We see that the I<method> calling type is almost twice slower, than
I<function>.  0.85 CPU compared to 1.59 CPU real execution time.  Why
...
find the module it belongs to and then the actual method.  Functions
form has one parameter less to pass, less stack operations, less time
to get to the guts of the subroutine.

=head2 The Overhead with Heavy Subroutines
...
relative overhead of the method call.  The next bechmark proves this
point:
...
significant one. If your goal is speed you might consider to use the
...
=head2 Are All Methods Slower than Functions?

Some modules' API is misleading, for example C<CGI.pm> allows you to
execute its subroutined as functions and methods. As you will see in a
moment its function form of the calls is slower than the method form
because it does some woodoo work when the function form call is used.
...
=head1 Imported Symbols and Memory Usage

There is a real memory hit when you import all of the function into
...
If you are looking for performance improvement, you will have to face
...
Request Library|modules/Apache_Request_libapreq_Gen>.  Its is written
in C, giving it a significant memory and performance benefit.  It has
all the functionality of C<CGI.pm> except HTML generation functions.




=head1 Sending Plain HTML as Compressed Output
...
=head1 Increasing Shared Memory With mergemem

C<mergemem> is an experimental utility for linux, which looks *very*
...
B<If you have tried this utility, please let us know what do you think
about it! Thanks>
